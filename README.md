...
> \*\*Lyam\*\* est un bot Discord complet, moderne et modulaire écrit en JavaScript (\*\*discord.js v14\*\*). Il combine un module d'\*\*Astronomie \& Espace\*\* extrêmement poussé (APOD, suivi ISS, éruptions solaires, télescope James Webb, etc.), un système complet de \*\*Modération \& Logs\*\*, un système d'\*\*XP \& Niveaux\*\*\[cite: 4, 8], un créateur de \*\*Sondages interactifs\*\*\[cite: 3], ainsi que divers \*\*Jeux\*\* et \*\*Utilitaires\*\*\[cite: 1].
...
- 🪐 \*\*Astronomie \& Espace complet\*\* : Intégration directe des API NASA (APOD, Mars Rover, NEO)\[cite: 1, 2, 7], NOAA SWPC (Météo spatiale, Éruptions solaires)\[cite: 1, 2, 7] et suivi en temps réel de l'ISS avec rendu de carte HTTP local via Express\[cite: 1, 6].

- 🛡️ \*\*Modération \& Logging avancé\*\* : Sanctions (`/ban`, `/kick`, `/mute`, `/warn`, `/purge`)\[cite: 1], registre d'historique SQLite\[cite: 1, 9] et salon de logs automatisé (`ModLogger`)\[cite: 1].

- ⭐ \*\*Système d'XP \& Niveaux\*\* : Gain d'XP par message (cooldown de 60s)\[cite: 8], génération de carte de classement (`/rank`)\[cite: 4] et configuration sur-mesure (`/xpconfig`)\[cite: 1, 11].

- 📊 \*\*Sondages \& Notifications\*\* : Sondages à expiration automatique (`PollChecker`)\[cite: 3], modes de vote unique/multiple\[cite: 3] et publications quotidiennes automatisées (Cron APOD/JWST)\[cite: 5].

- 🌐 \*\*Mini-serveur HTTP Express\*\* : Serveur intégré pour héberger et servir dynamiquement les éléments visuels de la carte ISS\[cite: 1, 6].
...
- \*\*Base de Données\*\* : `better-sqlite3` (Mode WAL activé)\[cite: 5]

- \*\*Serveur Web\*\* : Express.js\[cite: 1, 6]

- \*\*Tâches Planifiées\*\* : `node-cron`\[cite: 5]

- \*\*Requêtes HTTP\*\* : `axios`\[cite: 6, 12]

- \*\*Variables d'Environnement\*\* : `dotenv`\[cite: 2, 8]
